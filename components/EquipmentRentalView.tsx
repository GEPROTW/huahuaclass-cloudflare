...
import { Box, Plus, Trash2, Edit2, Check, X, Clock, User, ChevronLeft, ChevronRight, MapPin } from 'lucide-react';
...
export const EquipmentRentalView: React.FC<EquipmentRentalViewProps> = ({ 
    ...
    const teachingAids = systemConfig.teachingAids || [];
    const [selectedAidId, setSelectedAidId] = useState<string | null>(teachingAids.length > 0 ? teachingAids[0].id : null);
    
    // Edit state
    const [editingAidId, setEditingAidId] = useState<string | null>(null);
    const [editAidName, setEditAidName] = useState('');
    ...
    const handleSaveAid = (id: string) => {
        if (!editAidName.trim()) return;
        const newAids = teachingAids.map(a => a.id === id ? { ...a, name: editAidName.trim() } : a);
        onUpdateSystemConfig({ ...systemConfig, teachingAids: newAids });
        setEditingAidId(null);
    };

    const handleAddAid = () => {
        if (!newAidName.trim()) return;
        const newAid = { id: crypto.randomUUID(), name: newAidName.trim() };
        const newAids = [...teachingAids, newAid];
        onUpdateSystemConfig({ ...systemConfig, teachingAids: newAids });
        setIsAdding(false);
        setNewAidName('');
        if (!selectedAidId) setSelectedAidId(newAid.id);
    };

    const handleDeleteAid = (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (confirm('確定要刪除此教具嗎？')) {
            const newAids = teachingAids.filter(a => a.id !== id);
            onUpdateSystemConfig({ ...systemConfig, teachingAids: newAids });
            if (selectedAidId === id) {
                setSelectedAidId(newAids.length > 0 ? newAids[0].id : null);
            }
        }
    };

    const getLessonsForAid = (aidId: string) => {
        return lessons.filter(l => l.teachingAidIds?.includes(aidId));
    };
                ...
                <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg font-bold text-slate-800 flex items-center">
                            <Box className="w-5 h-5 mr-2 text-fuchsia-500" />
                            教具列表
                        </h2>
                        {!readOnly && (
                            <button 
                    ...
                    <div className="space-y-2 max-h-[calc(100vh-250px)] overflow-y-auto custom-scrollbar">
                        {isAdding && (
                            <div className="flex items-center gap-2 mb-2 bg-fuchsia-50 p-2 rounded-xl border border-fuchsia-100">
                                <input 
                                    className="flex-1 min-w-0 bg-white border border-fuchsia-200 rounded px-2 py-1 text-sm outline-none focus:border-fuchsia-400"
                                    value={newAidName}
                                    onChange={e => setNewAidName(e.target.value)}
                                    placeholder="教具名稱..."
                                    autoFocus
                                    onKeyDown={e => { if (e.key === 'Enter') handleAddAid(); if (e.key === 'Escape') setIsAdding(false); }}
                                />
                                <button onClick={handleAddAid} className="p-1 text-emerald-600 hover:text-emerald-700 flex-shrink-0 bg-emerald-100 hover:bg-emerald-200 rounded"><Check className="w-4 h-4" /></button>
                        ...
                        {teachingAids.length === 0 && !isAdding && (
                        ...
                        {teachingAids.map(aid => (
                            <div 
                                ...
                                className={`group flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${selectedAidId === aid.id ? 'bg-fuchsia-600 text-white shadow-md' : 'bg-slate-50 text-slate-700 hover:bg-fuchsia-50 hover:text-fuchsia-700 border border-transparent hover:border-fuchsia-100'}`}
                            >
                                {editingAidId === aid.id ? (
                                    <div className="flex items-center gap-2 w-full" onClick={e => e.stopPropagation()}>
                                        <input 
                                            className="flex-1 min-w-0 bg-white border border-fuchsia-300 text-slate-800 rounded px-2 py-1 text-sm outline-none"
                                            value={editAidName}
                                            onChange={e => setEditAidName(e.target.value)}
                                            autoFocus
                                            onKeyDown={e => { if (e.key === 'Enter') handleSaveAid(aid.id); if (e.key === 'Escape') setEditingAidId(null); }}
                                        />
                                        <button onClick={() => handleSaveAid(aid.id)} className="p-1 text-emerald-600 hover:text-emerald-700 flex-shrink-0 bg-emerald-100/50 hover:bg-emerald-200/50 rounded"><Check className="w-4 h-4" /></button>
                                        <button onClick={() => setEditingAidId(null)} className="p-1 text-slate-400 hover:text-slate-600 flex-shrink-0 bg-slate-200/50 hover:bg-slate-300/50 rounded"><X className="w-4 h-4" /></button>
                                    </div>
                                ) : (
                                    <>
                                        ...
                                        {!readOnly && (
                                            <div className={`flex items-center opacity-0 group-hover:opacity-100 transition-opacity ${selectedAidId === aid.id ? 'text-fuchsia-200' : 'text-slate-400'}`}>
                                                <button onClick={(e) => { e.stopPropagation(); setEditingAidId(aid.id); setEditAidName(aid.name); }} className="p-1 hover:text-white transition-colors">
                                                    <Edit2 className="w-3.5 h-3.5" />
                                                </button>
                                                <button onClick={(e) => handleDeleteAid(aid.id, e)} className="p-1 hover:text-red-400 transition-colors">
            ...
            <div className="flex-1 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col min-w-0">
                {selectedAidId ? (
                        ...
                        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
                            <div>
                                <h3 className="text-xl font-bold text-slate-800">
                                    {teachingAids.find(a => a.id === selectedAidId)?.name} 租借排程
                                </h3>
                                <p className="text-sm text-slate-500 mt-1">查看此教具在本週的使用狀況</p>
                            </div>
                            ...
                            <div className="flex items-center gap-1 sm:gap-2 bg-slate-50 p-1.5 rounded-xl border border-slate-200 shadow-sm">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    />
                                    <button onClick={handleToday} className="px-2 sm:px-3 py-1 font-bold text-xs bg-fuchsia-100 text-fuchsia-700 hover:bg-fuchsia-200 rounded-lg transition-all" title="回到本週">
                                        本週
                                    </button>
                                </div>
                            ...
                            <div className="min-w-[800px] h-full flex flex-col">
                                ...
                                <div className="grid grid-cols-7 gap-2 mb-4 border-b border-slate-100 pb-2">
                                    {weekDays.map((d, i) => {
                                        ...
                                        return (
                                            <div key={i} className={`flex flex-col items-center justify-center p-2 rounded-xl ${isToday ? 'bg-fuchsia-50 border border-fuchsia-100' : ''}`}>
                                                <span className={`text-xs font-bold ${isToday ? 'text-fuchsia-600' : 'text-slate-500'}`}>{dayNames[d.getDay()]}</span>
                                                <span className={`text-lg font-black mt-1 ${isToday ? 'text-fuchsia-700' : 'text-slate-700'}`}>{d.getDate()}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                                ...
                                <div className="flex-1 grid grid-cols-7 gap-2 relative min-h-[300px]">
                                    {weekDays.map((dateObj, colIndex) => {
                                        ...
                                        const dayLessons = getLessonsForAid(selectedAidId).filter(l => l.date === dateStr).sort((a,b) => a.startTime.localeCompare(b.startTime));
                                                ...
                                                {dayLessons.map(lesson => {
                                                    ...
                                                    return (
                                                        <div 
                                                            ...
                                                            className="bg-white border-l-4 border-fuchsia-500 p-2.5 rounded-lg shadow-sm hover:shadow-md transition-shadow text-xs group relative overflow-hidden cursor-pointer"
                                                        >
                                                            ...
                                                            <div className="font-bold mb-1 truncate text-fuchsia-700">{lesson.title}</div>
                    ...
                    <div className="h-full flex items-center justify-center flex-col text-slate-400">
                        ...
                        <h4 className="text-xl font-bold text-slate-500 mb-2">選擇或新增一個教具</h4>
                        <p>左側點擊教具以查看其租借狀況</p>
                    </div>
                )}
            </div>
